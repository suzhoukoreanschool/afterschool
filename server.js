...
app.post('/api/register', (req, res) => {
    ...
    const isDuplicate = backupRegistrations.some(r => r.studentId === studentId && r.className === className);
    if (isDuplicate) return res.status(400).json({ success: false, message: "이미 신청 완료된 강좌입니다." });
    
    backupRegistrations.push({ studentId, studentName, className, timestamp: new Date().toISOString() });
...
app.listen(PORT, () => console.log(`⚡ 완전 통합 서버 정상 구동중`));
